...
uniform edge weights. This paper examines whether structured edge
weights amplify that advantage. Seven experiments
...
to $6.1\times$ at 1{,}000 nodes, confirmed by permutation control
($p = 0.001$ versus shuffled assignment). The mechanism is bottleneck
...
$p \approx 2.5 \times 10^{-5}$ against an exhaustive null, while spectral analysis
...
\smallskip\noindent\textbf{MSC 2020:} 05C50 (primary), 05C22, 05C85, 52B10
...
For reference, Paper~1's key metrics: FCC lattices achieve 30--32\%
shorter average paths, 40\% smaller diameter, and a
$2.3$--$2.5\times$ Fiedler ratio under uniform weights, stable across
125--8{,}000 nodes.
...
After min-max normalization to $[0,1]$, the distribution exhibits
high variance (coefficient of variation $\approx 0.92$) and a heavy
right tail (skewness $\approx 1.8$), properties shared with many
real-world weight distributions. All lattice-scale findings
(amplification, bottleneck resilience) can be independently verified
using the random and power-law distributions, which show the same
amplification gradient at lower magnitude.
...
\begin{enumerate}
  \item \textbf{Amplification:} Structured edge weights amplify the
    ...
    to $6.1\times$ (direction-weighted corpus), more than doubling the
    Paper~1 baseline.
  \item \textbf{Mechanism:} The amplification arises from bottleneck
    resilience---FCC's 12-connected topology routes around
    near-disconnection bottlenecks that limit the 6-connected
    cubic lattice.
  ...
  \item \textbf{Prime-vertex coherence (exploratory):} A targeted prime-to-vertex
    mapping on the rhombic dodecahedron achieves $p \approx 2.5 \times 10^{-5}$
    against an exhaustive null of 40{,}320 permutations.
\end{enumerate}

\paragraph{Organization.}
Section~\ref{sec:background} reviews weighted algebraic connectivity
and FCC direction structure. Section~\ref{sec:method} describes the
two experimental registers and weight distributions.
Section~\ref{sec:lattice} presents the lattice-scale experiments
(edge-cycled and direction-weighted), including the amplification
mechanism. Section~\ref{sec:cell} presents the single-cell
characterization experiments (TV optimization, spectral analysis,
prime-vertex mapping, cross-polytope comparison).
Section~\ref{sec:discussion} discusses the mechanism, connections to
network resilience theory, and limitations.
...
minimum cut weight~\cite{mohar1991, chung1997}. Effective
resistance, the dual spectral quantity, measures how well a graph
distributes current between node
pairs~\cite{ghosh2008resistance}; graphs that maximize
$\lambda_2$ simultaneously minimize worst-case effective
resistance. Spectral sparsification~\cite{spielman2004} preserves
these spectral properties in reduced graphs, confirming that
$\lambda_2$ captures global connectivity robustly. Small
$\lambda_2$ indicates a low-conductance bottleneck; large
$\lambda_2$ indicates that no cheap partition exists.
...
CPU-bound). 312 tests verify library correctness. Source code and
...
\noindent\footnotesize{Random and power-law distributions were not computed
at scale 8{,}000 due to the $O(n^3)$ cost of Laplacian eigendecomposition.
The monotonic gradient (uniform $<$ random $<$ power-law $<$ corpus) is
established at both smaller scales.}
\end{table}
...
\noindent\footnotesize{Random and power-law not computed at scale 8{,}000
(same cost constraint as Table~\ref{tab:exp1}).}
\end{table}

Direction-based weighting more than doubles the Fiedler advantage:
from 2.55 (uniform) to 6.11 (corpus) at scale 1{,}000
(Figure~\ref{fig:amplification}). Both ratios are deterministic
(identical across 100 seeds); the random distribution produces
$3.94 \pm 0.46$ (95\% CI $[3.23, 4.90]$), confirming that the
corpus amplification substantially exceeds stochastic variation.
This is the largest effect in either paper.
...
compensates; at large scale, the per-neighbor dilution dominates. This matches the unweighted
...
annealing~\cite{kirkpatrick1983} with 100 restarts of 5{,}000
iterations each.
...
values versus the RD's 8.12. The bipartite constraint imposes a
structural cost on sorting, not an advantage---and this same
constraint is what produces the 12-connected tessellation.
...
corpus     & 0.0863 & 0.0313 & 2.7547 & 14 \\
\midrule
...
than hypothesis-confirmatory testing. The scoring rubric was designed
to capture multiple levels of prime divisibility; different rubric
choices would yield different optimal mappings and $p$-values.
...
\midrule
\textbf{Optimal} (geometry-determined) & \textbf{35.0} & $\mathbf{2.5 \times 10^{-5}}$ & 1 / 40{,}320 \\
...
The optimal mapping is the single highest-scoring permutation out of
all $8! = 40{,}320$ trials ($p \approx 1/40{,}320 = 2.5 \times 10^{-5}$).
Five of eight primes have direct divisibility hits in their assigned
vertex star:
\begin{itemize}
  \item $67 \to$ vertex 7: one edge $= 2 \times 67$
  \item $29 \to$ vertex 0: one edge $= 15 \times 29$
  \item $19 \to$ vertex 4: one edge $= 7 \times 19$
  \item $17 \to$ vertex 3: one edge $= 8 \times 17$
  \item $11 \to$ vertex 5: one edge $= 5 \times 11$
\end{itemize}
...
topology advantage compounds as a result. Independent validation
comes from signal processing: Fu et al.~\cite{fu2009rhombic}
demonstrated that the rhombic dodecahedron map provides an optimal
scheme for encoding panoramic video, outperforming cubic projections
in sampling uniformity and distortion. Their finding that the RD's
face-pair geometry yields superior spherical data mapping confirms
that the spectral advantage measured here extends to practical
...
\begin{itemize}
  \item The RD's bipartite constraint restricts optimal assignment
    (TV~=~8.12 vs.\ 6.80 for a random graph). The bipartite
    structure imposes a structural cost, not a sorting advantage.
  \item The prime-vertex mapping ($p \approx 2.5 \times 10^{-5}$) shows real
...
\subsection{Connections to Network Resilience Theory}
\label{sec:resilience}

The bottleneck resilience mechanism has parallels in network
resilience theory~\cite{dorogovtsev2008}. Ashby's Law of Requisite
Variety~\cite{ashby1956} predicts that a controller must match the
variety of disturbances it faces; heterogeneous weights increase
perturbation variety, and FCC's 12-connected topology provides more
alternative paths than SC's 6. Beer's Viable System
Model~\cite{beer1972} identifies axis-fragility as a failure mode:
the SC lattice is axis-aligned and a single weak direction pair
eliminates a third of connectivity, while FCC distributes
connectivity across 6 diagonal directions. These frameworks are
interpretive---they suggest \emph{why} the measured amplification
arises but do not constitute independent evidence.

\begin{table}[H]
\centering
\caption{Amplification summary: Paper~1 baseline versus Paper~2
  best result. Scale noted per metric.}
\label{tab:summary}
\begin{tabular}{lrrl}
\toprule
...
Consensus speedup  & 0.93$\times$ (scale 1000) & 6.69$\times$ (scale 125) & Direction-weighted corpus \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Deterministic ratios, stochastic control.} The Fiedler
ratios for uniform, power-law, and corpus distributions are
deterministic: given the weight assignment and lattice construction,
they produce identical values across all tested seeds ($n = 100$).
Multi-seed validation confirms the headline ratios exactly
(e.g., $6.11\times$ corpus at scale 1{,}000 across all 100 seeds).
Only the random distribution introduces variance:
$3.94 \pm 0.46$ (95\% CI $[3.23, 4.90]$) at scale 1{,}000 under
direction-based weighting across 100 seeds. Consensus dynamics are
seed-dependent and would benefit from additional validation at
intermediate scales.
...
The direction-pair structure suggests a natural application to
low-rank adapters (LoRA;~\citealt{hu2022lora}) for large language models: a learnable
bridge matrix with 6 channels following the RD's face-pair topology
could add cross-rank mixing at negligible parameter cost.
Results on this ``RhombiLoRA'' architecture are reported in the
companion paper~\cite{bielec2026bridge}.
...
\cite{petersen1962}. Whether it shows an intermediate weighted
amplification would help attribute the FCC advantage to connectivity
count versus isotropy.

\subsection{Consensus Dynamics Under Multi-Seed Validation}

The Fiedler ratios are deterministic across seeds, but consensus
dynamics (which involve random initial states) would benefit from
multi-seed validation, particularly near the inversion transition.
...
more than double the Paper~1 baseline), and arises from bottleneck
resilience: FCC's 12-connected topology routes around bottlenecks
that constrain cubic lattices.

At single-cell scale, the corpus values show real arithmetic
structure (prime-vertex coherence at $p \approx 2.5 \times 10^{-5}$) but
