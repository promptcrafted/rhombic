...
systems---images, volumes, and spatial data structures. This paper
presents a systematic empirical comparison of the simple cubic
...
provides 29--31\% shorter average paths, 38--42\% smaller diameter,
and 2.3--2.5$\times$ higher algebraic connectivity; 55\% greater
flood fill reach and 17\% faster nearest-neighbor queries; 5--10$\times$
lower signal reconstruction MSE with 5--20$\times$ more isotropic
error; and +7--20 percentage points higher embedding recall at 1-hop
in a proof-of-concept ANN index. The cost is bounded: approximately
$2\times$ the edge count ($12/6$). Across all tested scales, these ratios are stable,
consistent with derivation from Voronoi cell geometry rather than
sample size. An open-source library (\texttt{rhombic}) and interactive demo
...
the widely adopted default of digital computation. Memory addresses form
one-dimensional cubes, pixel grids form two-dimensional cubes, and
voxel spaces form three-dimensional cubes. This convention aligns with Cartesian analytic geometry
(Descartes, 1637) and stored-program architecture designs, accumulating
through convention rather than demonstrated optimality.

The face-centered cubic (FCC) lattice achieves the densest sphere
packing in three dimensions (together with HCP; Kepler's conjecture, announced by Hales in 1998 and
published in 2005~\cite{hales2005}). Its Voronoi cells are \textbf{rhombic
...
appears in crystal structures (Cu, Al, Au) because physics selects
for close-packed arrangements. Petersen and Middleton~\cite{petersen1962}
...
\begin{enumerate}
  ...
  \item An open-source library, \texttt{rhombic}, with 79 tests
...
close-packed) achieve this density. Hales announced a proof in
1998, published it in 2005~\cite{hales2005}, and the proof was
formally verified by the Flyspeck project in
...
(the rhombic dodecahedron) provides the most isotropic
tessellation among face-transitive polyhedra, and its 12-connected
...
PyPI: \url{https://pypi.org/project/rhombic/};
Demo: \url{https://huggingface.co/spaces/timotheospaul/rhombic}}
...
\texttt{python~-m~rhombic.benchmark} at commit
\texttt{v0.1.2} (PyPI), commit
\texttt{1cdb257ed3823b35bf92827c29754f8d42d5ab30}.\footnote{For benchmarks involving random
evaluation points, the seed ensures bitwise reproducibility on the
same platform. Cross-platform floating-point differences may
...
\begin{table}[H]
\centering
\caption{Graph theory benchmarks. Ratios are FCC/Cubic for
connectivity-type metrics and Cubic/FCC for path-type metrics
(higher is better in both cases).}
\label{tab:graph}
\begin{tabular}{lrrrrr}
\toprule
Scale & Nodes (SC/FCC) & ASPL ratio & Diam.\ ratio
  & Alg.\ conn.\ ratio & Edge ratio \\
\midrule
${\sim}125$  & 125 / 108   & 0.69 (31\%$\downarrow$)
  & 0.58 (42\%$\downarrow$) & 2.31$\times$ & 1.50$\times$ \\
${\sim}1000$ & 1000 / 864  & 0.68 (32\%$\downarrow$)
  & 0.59 (41\%$\downarrow$) & 2.55$\times$ & 1.61$\times$ \\
${\sim}4000$ & 4096 / 4000 & 0.71 (29\%$\downarrow$)
  & 0.62 (38\%$\downarrow$) & 2.43$\times$ & 1.88$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The ASPL advantage is stable at 29--32\% across all scales
tested. The algebraic connectivity ratio (2.3--2.5$\times$)
exceeds the raw degree ratio (12/6 = 2$\times$), indicating
that the \emph{isotropy} of the FCC lattice compounds the
...
\midrule
Flood fill reach & $+55\%$ & FCC reaches more per hop \\
NN query speed & $17\%$ faster & FCC resolves quicker \\
Spatial hash query & $6\%$ faster & FCC lookups faster \\
Sphere query density & $+24\%$ nodes & FCC returns more per sphere \\
Range query time & $3$--$5\times$ slower & FCC pays density cost \\
\bottomrule
\end{tabular}
\end{table}
...
matched-count control is noted as future work.
...
stability is consistent with an advantage that derives from Voronoi
cell geometry rather than boundary effects or sample size. At scales
well above the boundary layer, the ratios are determined by the
coordination number (12 vs.\ 6), the kissing number configuration,
and the Voronoi cell shape---constants of the lattice. Whether
...
5--10$\times$ signal fidelity $+$ 7--20pp embedding recall).

The FCC lattice (with HCP) achieves the densest sphere packing and
the topology that nature selects for close-packed structures. The SC
...
reconsidering that inheritance.

\paragraph{Availability.}
Code, data, and interactive demo:
